...
//    char *str_num;//why does this definition not work?
    String str="E:\\WavingTrees\\b00";//string,the 's' can be a captial or lower-caseletters

    /****read the first image,and reset the array w,u,sigma****/
    img=imread("E:\\WavingTrees\\b00000.bmp");
    if(img.empty())                
        return -1;
	namedWindow("src");
	imshow("src",img);waitKey(1);
...

    {
        if(!pause)
        {
            ...
            if(img.empty())
                break;
            str="E:\\WavingTrees\\b00";//after read,str must be reseted ;
